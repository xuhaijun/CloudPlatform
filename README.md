...
| [测试说明](docs/test/DSSAD云平台测试说明.md) | 16 类 / 200 用例逐类详述、覆盖率与门槛、未覆盖清单 |
...
| `F-01` | 性能报告 | **限流器未接入请求链路**（P0，未修复）—— 配置 100 次/分钟，实测 130 次全部放行 |
...
**尚未修复的高优先级项**：`F-01`（限流器未接入链路）、`P-01`/`O-07`（多实例 `@Scheduled` 无分布式锁）。
...
本项目已累积 5 例同类缺陷 —— `F-01`（限流器配了但没接线）、`E-03`（覆盖率门槛声明了但没有 check 执行）、
