...
| 维度 | 是否限流 | 说明 |
|---|---|---|
| 单车辆 HTTP（100/分钟） | ✅ | `tryAcquireHttp(vin)` |
| 单车辆 MQTT 发布（10/秒） | ✅ | `tryAcquireMqttPublish(vin)` |
| 单车辆上行报文（入站） | ❌ | **未限流**——依赖入站队列容量保护（见 10 章 D-02） |
...
| **P-03** | **高** | **限流器完全未接入请求链路**（原描述为「入站报文无频率限流」，经《性能优化与压测报告》F-01 实测修正）：`RateLimiter` 的 `tryAcquireHttp` / `tryAcquireMqttPublish` 在全项目**零调用点**，仅被监控端点读取配置用于展示——即配置为「单车辆 HTTP 100 次/分钟、MQTT 10 条/秒」的限流**一条都未生效** | ① 一台异常车端可无上限占用消费线程与数据库写入能力，入站队列只剩「容量满丢弃最旧」兜底（会连带丢弃正常车辆的报文）；② 监控端点仍如实报告 `enabled: true`，运维看到的是「已开启」而实际「没接线」，属最危险的组合 | 新增 `RateLimitInterceptor` 并注册到 `/api/v1/**`（完整方案与配套测试见《性能优化与压测报告》5.5）；入站报文在 `MqttMessageRouter` 中按发送方 VIN 限流；监控端点补充「已拦截次数」计数器 |
